...
def create_model(session, Model_class, path, load_vec, load_vec2, load_vec3, load_vec4, config, id_to_char, id_to_word, logger):
    ...
    if ckpt and tf.train.checkpoint_exists(ckpt.model_checkpoint_path):
        ...
        if config["pre_emb"]:
            ...
            #update char emb		
            emb_weights_char = session.run(model.char_lookup.read_value())
            #print(emb_weights_char)
            #emb_weights_char = load_vec(config["emb_file"], id_to_char, config["char_dim"], emb_weights_char)
            #print (id_to_char[0])
            emb_weights_char = load_vec(config["emb_file"], config["emb_file2"],id_to_char, config["char_dim"], emb_weights_char)
            session.run(model.char_lookup.assign(emb_weights_char))

            #update word emb
            emb_weights_word = session.run(model.word_lookup.read_value())
            #print(id_to_word[0])
            #print(emb_weights_word[0])
            #print(id_to_word[1])
            #print(emb_weights_word[1])
            emb_weights_word = load_vec2(config["emb_file2"], id_to_word, config["word_dim"], emb_weights_word)
            session.run(model.word_lookup.assign(emb_weights_word))

            #update seg emb		
            emb_weights_seg = session.run(model.seg_lookup.read_value())
            #print(emb_weights_seg)
            emb_weights_seg = load_vec3(emb_weights_seg)
            session.run(model.seg_lookup.assign(emb_weights_seg))

            #update pos emb		
            emb_weights_pos = session.run(model.pos_lookup.read_value())
            #print(emb_weights_pos)
            emb_weights_pos = load_vec4(emb_weights_pos)
            session.run(model.pos_lookup.assign(emb_weights_pos))
            
            #print(emb_weights_word[0])
            #print(emb_weights_word[1])
            #print(session.run(model.embedding))
